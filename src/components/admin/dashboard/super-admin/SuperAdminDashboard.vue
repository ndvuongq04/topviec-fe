<template>
  <div class="space-y-8">
    ...
    <div v-else-if="superAdminData" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <SAUserGrowthChart :data="superAdminData.userGrowth" />
      <SARevenueBarChart :data="superAdminData.revenueByMonth" />
    </div>

    <!-- Recent Activities -->
    <div v-if="loading" class="h-96 bg-slate-100 dark:bg-white/5 animate-pulse rounded-xl"></div>
    <SARecentActivityTable v-else-if="superAdminData" :activities="superAdminData.recentActivities" />
...
import SAUserGrowthChart from './SAUserGrowthChart.vue';
import SARevenueBarChart from './SARevenueBarChart.vue';
import SARecentActivityTable from './SARecentActivityTable.vue';
