<template>
  <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
          ...
          >
            ...
            <!-- Việc làm -->
            <td class="px-6 py-4 text-sm font-semibold text-slate-700 dark:text-slate-300">{{ employer.jobs }}</td>
            ...
            <td class="px-6 py-4">
              <div class="flex items-center gap-1">
                <button
                  class="p-2 text-slate-400 hover:text-[#963131] hover:bg-[#963131]/5 rounded-lg transition-colors"
                  title="Xem chi tiết"
                  @click="$emit('view', employer)"
                >
                ...
                <button
                  ...
                  title="Chỉnh sửa"
                  @click="$emit('edit', employer)"
                >
                  <span class="material-symbols-outlined text-xl">edit</span>
                </button>

                <!-- Action phụ theo trạng thái -->
                <button
                  v-if="employer.status === 'active'"
                  class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
                  title="Khóa"
                  @click="$emit('action', { employer, action: 'block' })"
                >
                  <span class="material-symbols-outlined text-xl">block</span>
                </button>
                <button
                  v-else-if="employer.status === 'pending'"
                  class="p-2 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors"
                  title="Duyệt"
                  @click="$emit('action', { employer, action: 'approve' })"
                >
                  <span class="material-symbols-outlined text-xl">check_circle</span>
                </button>
                <button
                  v-else-if="employer.status === 'blocked'"
                  class="p-2 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors"
                  title="Mở khóa"
                  @click="$emit('action', { employer, action: 'unblock' })"
                >
    ...
    <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
      ...
      <div class="flex items-center gap-1">
        <button
          class="p-2 text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <template v-for="page in visiblePages" :key="page">
          ...
          <button
            v-else
            class="size-8 flex items-center justify-center rounded-lg font-bold text-sm transition-colors"
