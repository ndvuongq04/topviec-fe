...
async function onRespondToReport() {
  ...
  if (!canRespondToReport.value) {
    toast.warning(
      'Không thể xác nhận',
      'Chỉ báo cáo nhóm A ở trạng thái chờ nhà tuyển dụng mới có thể xác nhận đã sửa.',
    )
    return
  }
  ...
  try {
    ...
    toast.error('Lỗi', reportStore.error ?? 'Không thể xác nhận sửa tin.')
  } finally {
...
.cd-form__file-remove .material-symbols-outlined {
  font-size: 16px;
}
...
.cd-form__btn--primary:active {
  transform: scale(0.97);
}

.cd-form__btn--primary:disabled,
.cd-form__btn--ghost:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
...
.cd-form__btn--ghost:hover:not(:disabled) {
  background: #f8fafd;
}
</style>
